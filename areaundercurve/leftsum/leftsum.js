...
function leftsum() {
    ...
    do {
        ...
        i += 1;
    } while (rectPoints.slice(-1)[0] <= b-dx);
    ...
    // Draw left sum rectangles on graph
    for(var p =0; p < rectPoints.length-1; p++) {
        var leftTop = g.c2p(new Coordinate(rectPoints[p], f(rectPoints[p])));
        var rightTop = g.c2p(new Coordinate(rectPoints[p+1], f(rectPoints[p])));
